...
import { X, Lock, AlertTriangle, Printer, DollarSign, Loader2, Banknote, CreditCard, Smartphone } from "lucide-react";
import { zodResolver } from "@hookform/resolvers/zod";
import { useShallow } from "zustand/shallow";
import { useTheme } from "@/shared/hooks/useTheme";
import { useCashRegisterStore } from "../store/cash-register.store";
import { closeCashRegisterSchema, type CloseCashRegisterSchema } from "../schemas";
import { useCloseCashRegister } from "../hooks";
import { formatDate, formatTime, formatCurrency, formatDifference } from "../helpers";
 ...
 */
export const FormCloseCashModal = ({ cashRegister }: FormCloseCashModalProps) => {
  // * Zustand
  const closeCloseCashModal = useCashRegisterStore(useShallow((state) => state.closeCloseCashModal));
  ...
  // * React Hook Form
  const { control, handleSubmit, watch, formState: { errors, isValid } } = useForm<CloseCashRegisterSchema>({
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div className={`${isDark ? 'bg-[#1E293B]' : 'bg-white'} rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl border-2 ${isDark ? 'border-red-500/30' : 'border-red-200'}`}>
        
        {/* Header */}
        <div className={`${isDark ? 'bg-gradient-to-r from-red-900/30 via-[#1E293B] to-red-900/30' : 'bg-gradient-to-r from-red-50 via-white to-red-50'} p-6 border-b ${isDark ? 'border-[#334155]' : 'border-gray-200'} rounded-t-3xl sticky top-0 z-10`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-2xl ${isDark ? 'bg-red-500/20 border border-red-500/30' : 'bg-red-100 border border-red-300'} shadow-lg`}>
                <Lock className={`w-7 h-7 ${isDark ? 'text-red-400' : 'text-red-600'}`} />
              </div>
              <div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          {/* Info General */}
          <div className={`${isDark ? 'bg-[#0F172A] border-[#334155]' : 'bg-gray-50 border-gray-200'} rounded-xl p-4 border-2`}>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Hora de Cierre:</span>
                <span className={`ml-2 font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {formatTime(new Date())}
                </span>
              </div>
              <div>
                <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Turno Iniciado:</span>
                <span className={`ml-2 font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {formatTime(cashRegister.openedAt)}
                </span>
              </div>
              <div>
                <span className={isDark ? 'text-gray-400' : 'text-gray-600'}>Total Órdenes:</span>
                <span className={`ml-2 font-medium ${isDark ? 'text-white' : 'text-gray-900'}`}>
          ...
          <div className="space-y-2">
            ...
            {errors.closingAmount && (
              <p className="text-red-500 text-xs font-medium">{errors.closingAmount.message}</p>
            )}
            
            {/* Mostrar diferencia calculada */}
            {closingAmountNumber > 0 && (
              <div className={`mt-2 p-3 rounded-lg ${Math.abs(calculatedDifference) < 0.01 ? 'bg-green-500/10' : calculatedDifference > 0 ? 'bg-blue-500/10' : 'bg-red-500/10'}`}>
                <div className="flex justify-between items-center">
                  <span className={`text-sm font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                    Diferencia:
                  </span>
                  <span className={`text-base font-bold ${differenceFormatted.color}`}>
          ...
          {/* Advertencia */}
          <div className={`${isDark ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-yellow-50 border-yellow-200'} border-2 rounded-xl p-4`}>
            <div className="flex items-start gap-3">
              <AlertTriangle className={`w-5 h-5 mt-0.5 ${isDark ? 'text-yellow-400' : 'text-yellow-600'}`} />
              <div>
                <h4 className={`font-bold text-sm ${isDark ? 'text-yellow-300' : 'text-yellow-800'}`}>
                  Importante
                </h4>
