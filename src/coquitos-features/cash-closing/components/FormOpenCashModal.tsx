...
import { X, Loader2, Unlock, Banknote } from "lucide-react";
...
export const FormOpenCashModal = () => {
  // * Zustand
  const closeOpenCashModal = useCashRegisterStore(
    useShallow((state) => state.closeOpenCashModal),
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center p-4 z-50">
      ...
      >
        {/* Header */}
        <div
          className={`${isDark ? "bg-gradient-to-r from-emerald-900/30 via-[#1E293B] to-emerald-900/30" : "bg-gradient-to-r from-emerald-50 via-white to-emerald-50"} p-6 border-b ${isDark ? "border-[#334155]" : "border-gray-200"} rounded-t-3xl`}
        >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
          ...
          >
            ...
            >
              💡 <strong>Consejo:</strong> Cuenta el efectivo inicial antes de
              abrir la caja. Este monto se usará para calcular la diferencia al
              cierre.
            </p>
          </div>
