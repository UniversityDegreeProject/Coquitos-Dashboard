...
import { Calculator, CheckCircle, Clock, DollarSign, ShoppingCart } from "lucide-react";
import { useTheme } from "@/shared/hooks/useTheme";
import { formatCurrency, formatDifference } from "../../helpers/format-currency";
import { formatDateTime, calculateDuration } from "../../helpers/format-datetime";
 ...
 */
export const CashRegisterHistoryItem = memo(({ cashRegister }: CashRegisterHistoryItemProps) => {
  ...
  const differenceFormatted = useMemo(() => {
    if (cashRegister.difference === null) return { text: "N/A", color: "text-gray-500" };
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-green-600">Cerrado</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div className="flex items-center gap-2">
          <ShoppingCart className={cn("w-4 h-4", isDark ? "text-gray-400" : "text-gray-500")} />
          <div>
            <p className={cn("text-xs", isDark ? "text-gray-400" : "text-gray-500")}>
              Órdenes procesadas
            </p>
      ...
      {/* Resumen financiero */}
      <div className={cn("rounded-lg p-4 mb-4", isDark ? "bg-[#0F172A]" : "bg-gray-50")}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className={cn("text-sm font-semibold", isDark ? "text-white" : "text-gray-900")}>
              {cashRegister.expectedAmount ? formatCurrency(cashRegister.expectedAmount) : "N/A"}
            </p>
          </div>
          <div>
            <p className={cn("text-xs mb-1", isDark ? "text-gray-400" : "text-gray-600")}>
            ...
            <p className={cn("text-sm font-semibold", isDark ? "text-white" : "text-gray-900")}>
              {cashRegister.closingAmount ? formatCurrency(cashRegister.closingAmount) : "N/A"}
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Notas si existen */}
      {cashRegister.notes && (
        <div className={cn("mt-4 pt-4 border-t", isDark ? "border-[#334155]" : "border-gray-200")}>
          <p className={cn("text-xs mb-1", isDark ? "text-gray-400" : "text-gray-600")}>
            Notas
          </p>
