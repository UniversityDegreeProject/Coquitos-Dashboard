...
export const CashClosePage = () => {
  ...
  const isOpenCashModalOpen = useCashRegisterStore(
    useShallow((state) => state.isOpenCashModalOpen)
  );
  const isCloseCashModalOpen = useCashRegisterStore(
    useShallow((state) => state.isCloseCashModalOpen)
  );
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {!cashRegister || cashRegister.status === "Cerrado" ? (
        <OpenCashAction />
      ) : (
        <CloseCashAction />
      )}
      ...
      {isCloseCashModalOpen && cashRegister && (
        <FormCloseCashModal cashRegister={cashRegister} />
      )}
    </div>
  );
};
