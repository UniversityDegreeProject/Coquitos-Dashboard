...
import type { GetCategoriesResponse } from "../interfaces";

interface CategoryPaginationsProps {
  paginationData: Omit<GetCategoriesResponse, "data">;
 ...
 */
export const CategoryPaginations = memo(({
  paginationData,
  onPageChange,
  onLimitChange,
  currentLimit,
  isLoading = false,
}: CategoryPaginationsProps) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-6">
        ...
        <div className={`text-sm ${isDark ? "text-[#94A3B8]" : "text-[#6B7280]"}`}>
          Total de categorías:{" "}
          <span className={`font-semibold ${isDark ? "text-[#F8FAFC]" : "text-[#1F2937]"}`}>
            {total} - 
          </span>{""}
          {" "}
          <span className={` ${isDark ? "text-[#F8FAFC]" : "text-[#1F2937]"}`}>
            pagina : {page}
          </span>{" "}
        </div>

          {/* Separador vertical */}
          <div className={`hidden sm:block w-px h-5 ${isDark ? "bg-[#475569]/50" : "bg-gray-300/50"}`} />

          {/* Selector de límite por página */}
          <div className="flex items-center gap-2">
            <label className={`text-sm ${isDark ? "text-[#94A3B8]" : "text-[#6B7280]"}`}>
              Límite de categorías por página:
            </label>
            <select
              value={currentLimit}
              ...
              {visiblePages.map((pageNum, index) => {
                if (pageNum === "...") {
                  return (
                    <span
                      key={`ellipsis-${index}`}
                      className={`px-2 ${isDark ? "text-[#94A3B8]" : "text-[#6B7280]"}`}
                    >
                      ...
                    </span>
                  ); 
...
CategoryPaginations.displayName = "CategoryPaginations";
