...
import { Loader2, ShoppingCart, User, CreditCard, Coins, StickyNote, AlertTriangle } from "lucide-react";
...
export const CheckoutForm = memo(({ 
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="space-y-4 flex-shrink-0">
      ...
      <button
        type="submit"
        disabled={isPending || !isValid || !isPaymentSufficient || cartItemsCount === 0 || !hasCashRegister}
        className={`w-full py-4 rounded-xl font-bold text-lg transition-all duration-300 flex items-center justify-center gap-2 ${
          isPending || !isValid || !isPaymentSufficient || cartItemsCount === 0 || !hasCashRegister
