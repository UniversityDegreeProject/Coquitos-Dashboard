...
export const PaymentMethodsChart = ({ report }: PaymentMethodsChartProps) => {
  ...
  const paymentMethods = [
    ...
    },
    {
      ...
      percentage:
        report.totalSales > 0 ? (report.salesByPaymentMethod.qr / report.totalSales) * 100 : 0,
      color: "bg-orange-500",
    },
  ];
    ...
    >
      ...
      <div className={`mt-4 pt-4 border-t ${isDark ? "border-slate-700" : "border-gray-200"}`}>
        <div className="flex justify-between items-center font-semibold text-lg">
          <span className={isDark ? "text-gray-300" : "text-gray-800"}>Total:</span>
          <span className="text-green-600">{formatCurrency(report.totalSales)}</span>
        </div>
      </div>
    </div>
  );
