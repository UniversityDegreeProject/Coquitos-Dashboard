  ...
  if (movements.length === 0) {
    return (
      ...
      >
        <div className="p-8 text-center">
          <ClipboardList
            className={`w-16 h-16 mx-auto ${colors.text.muted} opacity-50`}
          />
          <p className={`mt-4 text-lg ${colors.text.primary}`}>
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead
            className={`${isDark ? "bg-gradient-to-r from-gray-800 to-gray-700" : "bg-gradient-to-r from-gray-100 to-gray-50"}`}
          >
            <tr>
              <th
                className={`px-6 py-4 text-center text-xs font-bold ${colors.text.primary} uppercase tracking-wider border-b-2 ${isDark ? "border-blue-500" : "border-blue-600"}`}
              >
                Referencia
              </th>
              <th
            ...
            {movements.map((movement, index) => (
              ...
              >
                ...
                <td className={`px-6 py-4 text-center whitespace-nowrap`}>
                  <span
                    className={`text-sm font-bold ${isDark ? "text-blue-400" : "text-blue-500"}`}
                  >
                    {movement.reference}
                  </span>
                </td>
                ...
                <td className="px-6 py-4 text-center whitespace-nowrap">
                  <span
                    className={`px-3 py-1.5 inline-flex text-xs font-bold rounded-lg ${getTypeColor(movement.type)}`}
                  >
                    {movement.type}
                  </span>
                </td>
                ...
                <td className={`px-6 py-4 text-center whitespace-nowrap`}>
                  {(() => {
                    ...
                    return (
                      <span
                        className={`text-sm font-bold ${isIncrease ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                      >
                        {isIncrease ? "+" : ""}
                        {stockChange}
                      </span>
                ...
                {/* Stock Anterior */}
                <td
                  className={`px-6 py-4 text-center whitespace-nowrap text-sm font-medium ${colors.text.muted}`}
                >
                ...
                {/* Razón */}
                <td
                  className={`px-6 py-4 text-center text-sm ${colors.text.muted} max-w-xs`}
                >
                  <div className="flex flex-col items-center">
                ...
                {/* Fecha */}
                <td
                  className={`px-6 py-4 text-center whitespace-nowrap text-sm ${colors.text.muted}`}
                >
